# Fertility patterns across migrant generations (Finland, cohort 1985-94)

[![R language](https://img.shields.io/badge/language-R-blue)](https://www.r-project.org/)
[![NordForsk funding](https://img.shields.io/badge/funding-NordForsk-green)](https://www.nordforsk.org/projects/network-dynamics-ethnic-integration)
[![Research Council of Finland funding](https://img.shields.io/badge/funding-Research_Council_of_Finland-green)](https://research.fi/en/results/funding/81092)

This repository contains the code to replicate the results of the article:
- Estévez, J. L., & Rotkirch, A. (2026). 'Fertility trends across migrant generations reexamined: insights from Finnish register data'. _European Sociological Review_. https://doi.org/10.1093/esr/jcag009

## Purpose of the repository

This repository is dedicated to ensuring transparency and reproducibility in research on the fertility patterns of foreign-background populations in Finland.
It contains all the code used in the analysis presented in the accompanying article.
The data was provided by [Statistics Finland](https://stat.fi/) and can only be accessed with their authorization.

## Software requirements

## File list

The repository includes four R scripts and a data folder:
- [01_Data_retrieval.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/01_Data_retrieval.R)
- [02_Background reconstruction.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/02_Background%20reconstruction.R)
- [03_Person-year format and sample selection.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/03_Person-year%20format%20and%20sample%20selection.R)
- [04_Descriptive stats.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/04_Descriptive%20stats.R)
- [05_Matching.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/05_Matching.R)
- [06_Analyses.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/06_Analyses.R)
- [07_Analyses with frailty.R](https://github.com/joseluisesna/Fertility_migrant-generations_Finland/blob/main/07_Analyses%20with%20frailty.R)
 
## Instructions for use

## Funding

This research was supported by NordForsk through the project "The Network Dynamics of Ethnic Integration" (project number [105147](https://www.nordforsk.org/projects/network-dynamics-ethnic-integration)),
and by the Strategic Research Council (SRC) established within the Research Council of Finland (Decision Numbers: 
[364371](https://research.fi/en/results/funding/81099) and 
[364382](https://research.fi/en/results/funding/81092)).

## Citation

- Estévez, J. L., & Rotkirch, A. (accepted). 'Fertility Trends Across Migrant Generations Reexamined: Insights from Finnish Register Data'. _European Sociological Review_. https://doi.org/10.1093/esr/jcag009 

## Contact information

For any questions, please contact:
- José Luis Estévez (jose.estevez@helsinki.fi)
- Anna Rotkirch (anna.rotkirch@vaestoliitto.fi)
